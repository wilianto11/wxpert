...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Daftar Portfolio</h1>

        <!-- Button untuk membuka modal tambah portfolio -->
        <button type="button" class="bg-green-500 text-white px-4 py-2 rounded mb-3"
            onclick="toggleModal('addPortfolioModal')">
            Tambah Portfolio
        </button>
            ...
            <div class=" overflow-x-auto pb-4">
                <div class="block">
                                ...
                                @foreach ($portfolios as $index => $portfolio)
                                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td
                                            ...
                                            w-[200px] h-auto break-words whitespace-normal overflow-hidden">
                                            {{ $portfolio->description }}
                                        </td>
                                        <td class="w-48 px-5 py-3">
                                            <div class="flex items-center gap-3">
                                                <img src="{{ $portfolio->image ? asset('storage/' . $portfolio->image) : asset('assets/img/cat.png') }}"
                                                    alt="{{ $portfolio->title }}">
                                            </div>
                                        </td>
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                            <a href="{{ $portfolio->link }}" class="text-blue-600 hover:underline"
                                                target="_blank">
                                                {{ $portfolio->link }}
                                            </a>
                                        </td>
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                            <div
                                                ...
                                                <span
                                                    class="font-medium text-xs text-yellow-600 ">{{ $portfolio->category->name }}</span>
                                        </td>
                                        <td class="items-center p-5 gap-2">
                                            <div class="flex justify-center">
                                                ...
                                                <button
                                                    ...
                                                    onclick="toggleModal('editPortfolioModal{{ $portfolio->id }}')">
                    ...
                    <div id="addPortfolioModal"
                        ...
                        <div class="bg-white p-6 rounded-lg w-1/3">
                            <div class="flex justify-between items-center border-b pb-2">
                                <h5 class="text-lg font-bold">Tambah Portfolio</h5>
                                <button onclick="toggleModal('addPortfolioModal')" class="text-gray-500">&times;</button>
                            </div>
                            <div class="mt-4">
                                <form action="{{ route('admin.portfolios.store') }}" method="POST"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" name="image" id="image"
                                            class="w-full border p-2 rounded" accept="image/*" required>
                                    </div>
                    ...
                    @foreach ($portfolios as $portfolio)
                        <div id="editPortfolioModal{{ $portfolio->id }}"
                            class="fixed inset-0 hidden bg-gray-900 bg-opacity-50 flex items-center justify-center">
                            <div class="bg-white p-6 rounded-lg w-1/3">
                                <div class="flex justify-between items-center border-b pb-2">
                                    <h5 class="text-lg font-bold">Edit Portfolio</h5>
                                    <button onclick="toggleModal('editPortfolioModal{{ $portfolio->id }}')"
                                        class="text-gray-500">&times;</button>
                                </div>
                                <div class="mt-4">
                                    <form action="{{ route('admin.portfolios.update', $portfolio->id) }}" method="POST"
                                        ...
                                        <div class="mb-3">
                                            <label for="title" class="block font-semibold">Judul</label>
                                            <input type="text" name="title" id="title"
                                                class="w-full border p-2 rounded" value="{{ $portfolio->title }}"
                                                required>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <label for="slug" class="block font-semibold">Slug</label>
                                            <input type="text" name="slug" id="slug"
                                                class="w-full border p-2 rounded" value="{{ $portfolio->slug }}"
                                                required>
                                        </div>

                                        <div class="mb-3">
                                            <label for="description" class="block font-semibold">Deskripsi</label>
                                            <textarea name="description" id="description" class="w-full border p-2 rounded" required>{{ $portfolio->description }}</textarea>
                                        </div>
