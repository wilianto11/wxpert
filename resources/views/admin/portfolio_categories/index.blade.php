...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Portfolio Categories</h1>

        <!-- Button untuk membuka modal tambah kategori -->
        <button type="button" class="bg-green-500 text-white px-4 py-2 rounded-lg mb-3" onclick="openModal('add')">Tambah
            Kategori</button>

        <!-- Modal Tambah Kategori -->
        <div id="modalAdd" class="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center hidden">
            <div class="bg-white p-6 rounded-lg w-1/3 shadow-lg">
                ...
                <form action="{{ route('admin.portfolio_categories.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="slug" class="block text-sm font-medium text-gray-700">Slug Kategori</label>
                        <input type="text" id="slug" name="slug"
                            class="mt-1 block w-full border border-gray-300 rounded-lg p-2">
                    </div>
                    <div class="flex justify-end space-x-2">
                        <button type="button" onclick="closeModal('add')"
                            class="bg-gray-400 text-white rounded-lg px-4 py-2">Cancel</button>
                        <button type="submit" class="bg-blue-500 text-white rounded-lg px-4 py-2">Tambah</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div id="modalEdit" class="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center hidden">
            <div class="bg-white p-6 rounded-lg w-1/3 shadow-lg">
                ...
                <form id="editForm" action="" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="editSlug" class="block text-sm font-medium text-gray-700">Slug Kategori</label>
        ...
        <div id="modalDelete" class="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center hidden">
            <div class="bg-white p-6 rounded-lg w-1/3 shadow-lg">
                <h2 class="text-xl font-semibold mb-4">Hapus Kategori</h2>
                <p class="text-sm text-gray-700 mb-4">Apakah Anda yakin ingin menghapus kategori ini?</p>
                <form id="deleteForm" action="" method="POST">
                    ...
                    <div class="flex justify-end space-x-2">
                        <button type="button" onclick="closeModal('delete')"
                            class="bg-gray-400 text-white rounded-lg px-4 py-2">Cancel</button>
                        <button type="submit" class="bg-red-500 text-white rounded-lg px-4 py-2">Hapus</button>
                    </div>
                </form>
            </div>
        </div>

        <!-- Tabel daftar kategori portfolio -->
                ...
                <div class="block">
                    <div class="overflow-x-auto w-full border rounded-lg border-gray-300">
                        <table class="w-full rounded-xl">
                            <thead>
                                <tr class="bg-gray-50">
                                    <th class="p-5 text-left text-sm font-semibold text-gray-900">NO</th>
                                    <th class="p-5 text-left text-sm font-semibold text-gray-900">Nama Kategori</th>
                                    <th class="p-5 text-left text-sm font-semibold text-gray-900">Slug Kategori</th>
                                    <th class="p-5 text-left text-sm font-semibold text-gray-900">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($categories as $category)
                                    <tr class="bg-white hover:bg-gray-50">
                                        <td class="p-5 text-sm text-gray-900">{{ $loop->iteration }}</td>
                                        <td class="p-5 text-sm text-gray-900">{{ $category->name }}</td>
                                        <td class="p-5 text-sm text-gray-900">{{ $category->slug }}</td>
                                        <td class="p-5 flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                onclick="openEditModal({{ $category->id }}, '{{ $category->name }}', '{{ $category->slug }}')">
    ...
    <script>
        ...
        function openModal(type) {
            document.getElementById(`modal${capitalizeFirstLetter(type)}`).classList.remove('hidden');
        }

        // Function untuk menutup modal
        function closeModal(type) {
            document.getElementById(`modal${capitalizeFirstLetter(type)}`).classList.add('hidden');
        }

        // Function untuk membuka modal edit
        function openEditModal(id, name, slug) {
            const form = document.getElementById('editForm');
            form.action = `/admin/portfolio_categories/${id}`;
            document.getElementById('editName').value = name;
            document.getElementById('editSlug').value = slug;
            openModal('edit');
        }

        // Function untuk membuka modal hapus
        ...
        // Function untuk mengkapitalisasi kata pertama
